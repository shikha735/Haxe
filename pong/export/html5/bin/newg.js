...
Main.prototype = $extend(flash.display.Sprite.prototype,{
	...
	,init: function() {
		...
		var scoreFormat = new flash.text.TextFormat("Verdana",24,12303291,true);
		scoreFormat.align = "CENTER";
		this.scoreField = new flash.text.TextField();
		this.addChild(this.scoreField);
		this.scoreField.set_width(500);
		this.scoreField.set_y(30);
		this.scoreField.defaultTextFormat = scoreFormat;
		this.scoreField.set_selectable(false);
		var messageFormat = new flash.text.TextFormat("Verdana",18,12303291,true);
		messageFormat.align = "CENTER";
		this.messageField = new flash.text.TextField();
		this.addChild(this.messageField);
		this.messageField.set_width(500);
		this.messageField.set_y(400);
		this.messageField.defaultTextFormat = messageFormat;
		...
		this.ballSpeed = 7;
		this.blockSpeed = 7;
		this.blockMovement = new flash.geom.Point(225,250);
		this.ballMovement = new flash.geom.Point(0,0);
		this.setGameState(GameState.Paused);
		this.get_stage().addEventListener("keydown",$bind(this,this.keyDown));
		this.get_stage().addEventListener("keyup",$bind(this,this.keyUp));
	...
	,setGameState: function(state) {
		...
		if(state == GameState.Paused) this.messageField.set_alpha(1); else {
			...
			this.ballMovement.y = direction * Math.cos(randomAngle) * this.ballSpeed;
			this.ballMovement.x = Math.sin(randomAngle) * this.ballSpeed;
		}
	}
	,keyDown: function(event) {
		if(this.currentGameState == GameState.Paused && event.keyCode == 32) this.setGameState(GameState.Playing); else if(event.keyCode == 37) this.arrowKeyUp = true; else if(event.keyCode == 39) this.arrowKeyDown = true;
	}
	,keyUp: function(event) {
		if(event.keyCode == 37) this.arrowKeyUp = false; else if(event.keyCode == 39) this.arrowKeyDown = false;
	}
	,everyFrame: function(event) {
		if(this.currentGameState == GameState.Playing) {
			if(this.arrowKeyUp) {
				var _g = this.platform1;
				_g.set_x(_g.x - this.platformSpeed);
			}
			if(this.arrowKeyDown) {
				var _g1 = this.platform1;
				_g1.set_x(_g1.x + this.platformSpeed);
			}
			if(this.ball.y > 300 && this.ball.x > this.platform2.x + 70) {
				var _g2 = this.platform2;
				_g2.set_x(_g2.x + this.platformSpeed);
			}
			if(this.ball.y > 300 && this.ball.x < this.platform2.x + 30) {
				var _g3 = this.platform2;
				_g3.set_x(_g3.x - this.platformSpeed);
			}
			if(this.platform1.x < 5) this.platform1.set_x(5);
			if(this.platform1.x > 395) this.platform1.set_x(395);
			if(this.platform2.x < 5) this.platform2.set_x(5);
			if(this.platform2.x > 395) this.platform2.set_x(395);
			var _g4 = this.ball;
			_g4.set_y(_g4.y + this.ballMovement.y);
			var _g5 = this.ball;
			_g5.set_x(_g5.x + this.ballMovement.x);
			if(this.block1.x + 50 < this.ball.x - 10) {
				var _g6 = this.block1;
				_g6.set_x(_g6.x + this.blockSpeed);
			} else if(this.block1.x > this.ball.x + 10) {
				var _g7 = this.block1;
				_g7.set_x(_g7.x - this.blockSpeed);
			}
			if(this.ballMovement.y < 0 && this.ball.y < 30 && this.ball.x >= this.platform1.x && this.ball.x <= this.platform1.x + 100) {
				this.bounceBall();
				this.ball.set_y(30);
			}
			if(this.ballMovement.y > 0 && this.ball.y > 470 && this.ball.x >= this.platform2.x && this.ball.x <= this.platform2.x + 100) {
				this.bounceBall();
				this.ball.set_y(470);
			}
			if(this.ballMovement.y < 0 && (this.ball.y >= this.block1.x && this.ball.y < this.block1.x + 50)) this.bounceBall();
			if(this.ballMovement.y > 0 && (this.ball.x >= this.block1.x && this.ball.x < this.block1.x + 50)) this.bounceBall();
			if(this.ball.x < 5 || this.ball.x > 495) this.ballMovement.x *= -1;
			if(this.ball.y < 5) this.winGame(Player.AI);
			if(this.ball.y > 495) this.winGame(Player.Human);
		}
	}
	,bounceBall: function() {
		var direction;
		if(this.ballMovement.y > 0) direction = -1; else direction = 1;
		var randomAngle = Math.random() * Math.PI / 2 - 45;
		this.ballMovement.y = direction * Math.cos(randomAngle) * this.ballSpeed;
		this.ballMovement.x = Math.sin(randomAngle) * this.ballSpeed;
	}
...
var Ball = function() {
	flash.display.Sprite.call(this);
	this.get_graphics().beginFill(10066278);
	this.get_graphics().drawCircle(0,0,10);
	this.get_graphics().endFill();
};
$hxClasses["Ball"] = Ball;
Ball.__name__ = ["Ball"];
Ball.__super__ = flash.display.Sprite;
Ball.prototype = $extend(flash.display.Sprite.prototype,{
	__class__: Ball
});
var Block = function() {
	flash.display.Sprite.call(this);
	this.get_graphics().beginFill(16777215);
	this.get_graphics().drawRect(0,0,50,20);
	this.get_graphics().endFill();
};
$hxClasses["Block"] = Block;
Block.__name__ = ["Block"];
Block.__super__ = flash.display.Sprite;
Block.prototype = $extend(flash.display.Sprite.prototype,{
	__class__: Block
});
var openfl = {};
...
var Player = $hxClasses["Player"] = { __ename__ : ["Player"], __constructs__ : ["Human","AI"] };
...
var Obstacle = function() {
	flash.display.Sprite.call(this);
	this.get_graphics().beginFill(16777215);
	this.get_graphics().drawRect(0,0,25,30);
	this.get_graphics().endFill();
};
$hxClasses["Obstacle"] = Obstacle;
Obstacle.__name__ = ["Obstacle"];
Obstacle.__super__ = flash.display.Sprite;
Obstacle.prototype = $extend(flash.display.Sprite.prototype,{
	__class__: Obstacle
});
var Platform = function() {
	flash.display.Sprite.call(this);
	this.get_graphics().beginFill(16777215);
	this.get_graphics().drawRect(0,0,100,15);
	this.get_graphics().endFill();
};
...
ApplicationMain.config = { antialiasing : 0, background : 5374034, borderless : false, depthBuffer : false, fps : 60, fullscreen : false, height : 500, orientation : "portrait", resizable : true, stencilBuffer : false, title : "newg", vsync : true, width : 500};
